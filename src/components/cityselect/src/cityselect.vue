  ...
  <div>
    ...
    <div class="yd-cityselect" :class="show ? 'yd-cityselect-active' : ''">
      ...
      <ul v-show="ready" class="yd-cityselect-content" :class="activeClasses">
        <li
          class="yd-cityselect-item"
          v-for="index, key in columnNum"
          :ref="'itemBox' + index"
          :key="key"
        >
          <template
            v-if="columnsObj['columnItems' + index] && columnsObj['columnItems' + index].length > 0"
          >
...
export default {
  ...
  watch: {
    value(val) {
      if (isIOS) {
        val
          ? pageScroll.lock(this.$refs.mask.$el)
          : pageScroll.unlock(this.$refs.mask.$el);
      }
      this.show = val;
    },
    ready(val) {
